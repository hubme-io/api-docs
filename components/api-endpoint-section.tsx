...
import { Copy, Play, AlertCircle, CheckCircle, XCircle, Info, FileText, ChevronDown } from "lucide-react"
...
export function ApiEndpointSection({ endpointId }: ApiEndpointSectionProps) {
  ...
  if (!operation) {
    return (
      <div className="text-center py-8">
        ...
        <h2 className="text-xl font-semibold text-gray-600">Endpoint não encontrado</h2>
        <p className="text-gray-500">O endpoint selecionado não pôde ser carregado.</p>
      </div>
    )
  }
  ...
  const executeRequest = async () => {
    ...
    try {
      ...
      setResponse({
        ...
        data: result.data,
        requestUrl: `https://devapi.managefy.com.br/integration${targetPath}${queryParams.toString() ? `?${queryParams.toString()}` : ""}`,
  ...
  const getResponseSchema = () => {
    if (!operation.responses?.["200"]?.content?.["application/json"]?.schema) return null
      ...
      <div className="space-y-4">
        {Object.entries(schema.properties).map(([name, prop]: [string, any]) => (
          <div key={name} className="border-b border-gray-200 pb-4 last:border-0 last:pb-0">
            ...
            {prop.description && <p className="mt-1 text-sm text-gray-600">{prop.description}</p>}
            {prop.enum && (
              <div className="mt-2">
                <span className="text-xs font-medium text-gray-500">Valores permitidos:</span>
            ...
            {prop.example && (
              <div className="mt-2">
                <span className="text-xs font-medium text-gray-500">Exemplo:</span>
                <code className="ml-2 text-xs text-gray-800">{JSON.stringify(prop.example)}</code>
              </div>
            )}
          </div>
        ))}
    ...
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
          <Badge className={`font-mono w-fit ${getMethodColor(method)}`}>{method}</Badge>
          <code className="text-base sm:text-lg font-mono bg-gray-100 px-3 py-1 rounded break-all">{path}</code>
        </div>
        ...
        <div>
          ...
          {operation.description && <p className="text-gray-600 leading-relaxed">{operation.description}</p>}
        </div>
      </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
              <TabsList className="w-full rounded-none border-b px-6">
                ...
                <TabsTrigger value="response" className="text-sm">
                  ...
                  {response && (
                    <Badge
                      variant={response.error || response.status >= 400 ? "destructive" : "default"}
              ...
              <TabsContent value="request" className="flex-1 flex flex-col p-0 m-0">
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                      ...
                      <div className="space-y-4">
                        {operation.parameters.map((param: any) => (
                          <div key={param.name} className="space-y-2">
                            <Label htmlFor={param.name} className="flex flex-wrap items-center gap-2">
                            ...
                            <Input
                              id={param.name}
                              placeholder={param.schema?.example || param.description || `Digite ${param.name}`}
                  ...
                  {["POST", "PUT", "PATCH"].includes(method) && operation.requestBody && (
                    <div className="space-y-4">
                      <h3 className="text-sm font-semibold text-gray-700">Corpo da Requisição</h3>
                      <Textarea
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="border-b bg-gray-50 px-6 py-3 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Badge variant={response.error || response.status >= 400 ? "destructive" : "default"}>
                          {response.error ? "Erro" : response.status}
                        </Badge>
                        <span className="text-sm text-gray-600">{response.statusText}</span>
                      </div>
                      <span className="text-sm text-gray-500">{response.requestTime}ms</span>
                    </div>
                    ...
                    <Tabs defaultValue="body" className="flex-1 flex flex-col">
                      ...
                      <TabsContent value="headers" className="flex-1 p-0 m-0">
                        <div className="bg-gray-900 text-gray-100 h-full">
                          <div className="h-full overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent p-4">
                            <pre className="text-xs sm:text-sm">{JSON.stringify(response.headers, null, 2)}</pre>
                          </div>
                        </div>
                      </TabsContent>
                      ...
                      <TabsContent value="request" className="flex-1 p-0 m-0">
                        <div className="bg-gray-900 text-gray-100 h-full">
                          <div className="h-full overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent p-4 space-y-4">
                            <div>
                              <h4 className="font-semibold mb-2 text-gray-200">URL</h4>
                              <code className="text-xs sm:text-sm bg-gray-800 p-2 rounded block break-all">
                                {response.requestUrl}
                              </code>
                            </div>
                            <div>
                              <h4 className="font-semibold mb-2 text-gray-200">Cabeçalhos da Requisição</h4>
                ...
                <TabsContent value="errors" className="p-6 space-y-6">
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-semibold text-gray-700">400 Bad Request</h3>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      <XCircle className="h-4 w-4 text-red-600" />
                      <h3 className="text-sm font-semibold text-gray-700">401 Unauthorized</h3>
                    </div>
                    <div className="bg-red-50 p-4 rounded-lg">
                      <div className="max-h-40 overflow-y-auto scrollbar-thin scrollbar-thumb-red-300 scrollbar-track-transparent">
                        <pre className="text-xs sm:text-sm">
            ...
            <CardContent className="px-6 py-4 space-y-4">
              <Collapsible className="space-y-2">
                ...
                <CollapsibleContent className="text-sm text-gray-600 space-y-2">
                  ...
                  <p className="text-xs bg-gray-50 p-2 rounded font-mono">access-token: SEU_TOKEN_DE_ACESSO</p>
                </CollapsibleContent>
              </Collapsible>
